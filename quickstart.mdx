...
<Steps>
  <Step title="Quick install" titleSize="h2">
    The fastest way to install TuringDB, no Python or package manager required:

    ```bash
    curl https://raw.githubusercontent.com/turing-db/turingdb/refs/heads/main/scripts/install.sh | bash
    ```

    Prefer installing inside a Python project? Skip to the next steps for `uv` or `pip`.
  </Step>
