...
## DISTINCT, OPTIONAL MATCH, variable-length paths

```jsx
// De-duplicate rows
MATCH (n:Person) RETURN DISTINCT n.city

// Keep left side even with no match
OPTIONAL MATCH (a:Person)-[:KNOWS]->(b) RETURN a, b

// Variable-length paths (postfix quantifiers, not Neo4j's -[*1..3]-)
...
```jsx
MATCH (n) WHERE n.country IN ['UK', 'France'] RETURN n.name
...
Scalar functions work in both `RETURN` and `WHERE`; aggregates are `RETURN`-only.

| Function          | Description                                                        |
| ----------------- | ------------------------------------------------------------------ |
| `labels(n)`       | Node label as a string                                             |
| `type(e)` / `edgeType(e)` | Edge type as a string (aliases)                            |
| `keys(n)`         | Property keys of a node or edge                                    |
...
| `count(*)`, `min(e)`, `max(e)`, `avg(e)` | Aggregates (RETURN only)                     |

```jsx
// Introspection
MATCH (n)-[e]->(m) RETURN labels(n), type(e), keys(n)
