...
- **Edge types** with a colon `:` - e.g. `[e:FRIENDS_WITH]` or `[:FRIENDS_WITH]`
...
An edge carries exactly one edge type, so each edge pattern takes a single `[:EDGE_TYPE]` constraint. Use `CALL db.edgeTypes()` to list the edge types available in the graph.
...
```jsx
MATCH (n)-[e]->(m) RETURN labels(n), edgeType(e), labels(m)
```
...
| `LOAD JSONL 'mygraph.jsonl' AS my_graph WITH EMBEDDINGS [{"emb", 384}]` | Load JSONL and mark array properties as embeddings (see [JSONL import](/import_data/jsonl#loading-embedding-properties))                |
