...
TuringDB supports the `count()` and `avg()` aggregating functions.

Aggregate functions may only be used when the query has a single return item, e.g.

~~~
MATCH (n) RETURN count(n.age)
~~~

However, aggregates may be combined in a single return item, e.g.

~~~
MATCH (n) RETURN count(n) + avg(n.age)
~~~
