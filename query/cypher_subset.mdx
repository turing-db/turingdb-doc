...
Beyond equality, `WHERE` supports membership and substring predicates:

```jsx
// Membership
MATCH (n) WHERE n.country IN ['UK', 'France', 'Spain'] RETURN n.name

// Prefix / suffix / substring
MATCH (n) WHERE n.name STARTS WITH 'Al' RETURN n.name
MATCH (n) WHERE n.name ENDS WITH 'ice' RETURN n.name
MATCH (n) WHERE n.bio CONTAINS 'engineer' RETURN n.name
```

# Built-in Functions

TuringDB supports built-in functions for type conversion, introspection, and embedding comparison. These **scalar** functions can be used in both `RETURN` and `WHERE`.

## Introspection: `labels()`, `type()` / `edgeType()`, `keys()`

```jsx
MATCH (n)-[e]->(m) RETURN labels(n), type(e), labels(m)   // type(e) and edgeType(e) are aliases
MATCH (n) RETURN keys(n)                                   // property keys of a node or edge
```
...
MATCH (n) RETURN toBoolean("true")
```

## Embedding comparison: `cosine_similarity()`, `euclidean_distance()`

Compare two embeddings (embedding literals use parentheses — see [Vector Search](/vector-search)):

```jsx
MATCH (n:Document) RETURN n.title, cosine_similarity(n.emb, (0.4, 0.3, 0.8, 0.1))
MATCH (n:Document) RETURN n.title, euclidean_distance(n.emb, (0.4, 0.3, 0.8, 0.1))
```

## Aggregates: `count()`, `min()`, `max()`, `avg()`

Aggregate functions can only be used in `RETURN` — not in `WHERE`, `ORDER BY`, `SKIP`, or `LIMIT`.

```jsx
MATCH (n:Person) RETURN count(*)
MATCH (n:Person) RETURN min(n.age), max(n.age), avg(n.age)
```
...
A procedure's output columns can be `YIELD`ed and fed into a following `MATCH`/`WHERE`:
...
| `CREATE VECTOR INDEX <name> WITH DIMENSION <n> METRIC <COSINE\|EUCLID>` | Create a vector index (see [Vector Search](/vector-search))                                                                            |
...
| `MERGE_DATAPARTS`                        | Compact all DataParts of the current graph into one (see [DataParts](/concepts/dataparts))                                                            |
...
<Warning>
  `UNWIND` currently accepts **literal lists only** — `UNWIND $param`, `UNWIND someVariable`, and `UNWIND collect(...)` are not yet supported. List literals (`[...]`) may also be returned directly, e.g. `RETURN [1, 2, 3] AS nums`, but their elements must be literals.
</Warning>
