...
| **Durability**  | Committed data is preserved on disk once the graph is persisted (see note) |

_Based on the definition of Jim Gray & Andreas Reuter_ ([Gray & Reuter, 1993](https://archive.org/details/transactionproce0000gray))

<Note>
  TuringDB is an in-memory engine. Commits are durable once the graph is written to the TuringDB directory on disk (graphs loaded from / saved to `-turing-dir`); running with `-in-memory` keeps everything in RAM only. There is no write-ahead log, so durability follows the persistence model rather than a per-commit fsync.
</Note>
...
See also: [DataParts](/concepts/dataparts) and [Versioning](/concepts/versioning_system)
