...
To load Parquet files containing columns for node IDs, node properties, edge source and target IDs, and edge properties, see: [`LOAD PARQUET`](#load-parquet).
To load Parquet files containing a JSON string column, which is then converted into a graph structure, see: [`turing-parquet`](#turing-parquet).
