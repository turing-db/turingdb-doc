...
| `type` | Transport | Constructor arguments |
| ------ | --------- | --------------------- |
| `"json"` *(default)* | HTTP / REST against a running daemon | `host="http://localhost:6666"` |
| `"native"` | Native binary protocol (faster; requires the compiled extension) | `host="localhost"`, `port=6666` |
...
<Note>
  `list_available_graphs()` and the S3 helpers (`s3_connect`, `transfer`) are only supported on the default `json` backend.
</Note>
...
Creates a new isolated change (runs `CHANGE NEW`), makes it the active change, and returns its integer ID.
...
These helpers move graph data between local paths, S3, and the TuringDB server. They are only available on the default `json` backend.
...
- The default (`json`) backend uses `httpx` for HTTP requests; the `native` and `embedded` backends use a compiled extension
